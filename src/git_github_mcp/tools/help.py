    ...
    if topic in (None, "github_ops"):
        lines.extend(
            [
                ...
                "| show_repo         | owner, repo                   | output_format: markdown|html|json |",
                "| search_repos_topic| topic                         | owner (user scope), query (extra) |",
                "| code_find_repos   | extension or path_pattern or query | owner → user:OWNER; search_scope |",
                "| search_code       | query                         | pretty=True → markdown + unique_repos |",
                "| project_*         | owner (+ project_number)      | create/edit need title            |",
                "| package_*         | package_type, package_name    | owner=None → authenticated user |",
                "| gitingest_link     | owner, repo                   | ref, subpath (folder under ref)   |",
                "| gitingest_convert_url | github_url (or query)      | paste full github.com URL         |",
                "| gitingest_help     | —                             | markdown vs llms.txt / llms-full  |",
                "",
            ]
        )
    return "\n".join(lines)
