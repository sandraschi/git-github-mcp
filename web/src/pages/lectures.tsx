...
const CATEGORIES: { id: LectureCategory; label: string; Icon: typeof BookOpen }[] = [
...
const LECTURES: Lecture[] = [
  {
    ...
    category: 'git',
    title: 'Rebase',
    subtitle: 'Replay your work on top of a fresher base — linear history, extra care.',
    body: [
    ...
    tryThis: ['git fetch origin', 'git rebase origin/main', '# after fixing conflicts:', 'git rebase --continue', '# or bail out:', 'git rebase --abort'],
    watchOut: [
  ...
  },
  {
    ...
    subtitle: 'Two ways to combine branches — pick based on history politics, not dogma.',
    body: [
  ...
  },
  {
    ...
    watchOut: ['Context drift: a commit that applied cleanly on main may conflict on an older branch. Read the diff, don’t blindly trust the tool.'],
  },
  {
    key: 'revert-vs-reset',
    category: 'git',
    title: 'Revert vs reset',
    subtitle: 'Undo mistakes on shared history without gaslighting your teammates.',
    body: [
    ...
    tryThis: ['git revert <sha>', 'git reset --soft HEAD~1', 'git reset --hard <sha>  # local only!'],
    watchOut: ['Hard reset on shared branches forces others to recover manually. Prefer revert for anything already on GitHub.'],
  },
  {
    key: 'stash',
    category: 'git',
  ...
  },
  {
    ...
    category: 'git',
    title: 'Bisect',
    subtitle: 'Binary search through history to find which commit broke the build.',
    body: [
    ...
    tryThis: ['git bisect start', 'git bisect bad', 'git bisect good <last-known-good-sha>', '# test each step, then:', 'git bisect good # or bad', 'git bisect reset'],
  },
  {
    key: 'upstream',
    category: 'git',
  ...
  },
  {
    ...
    tryThis: ['git worktree add ../hotfix-main main', 'git worktree list', 'git worktree remove ../hotfix-main'],
  },
  {
    key: 'pr-lifecycle',
    category: 'github',
  ...
  },
  {
    ...
    tryThis: ['github_ops(operation="workflow_runs", owner="…", repo="…", limit=5)'],
    watchOut: ['Fleet ci_pulse only surfaces failures in the last 48h — older red builds need manual history.'],
  },
  {
    key: 'dependabot',
    category: 'github',
  ...
  },
  {
    ...
    subtitle: 'Boards and fields at org or user scope — not the same as repo Projects classic.',
    body: [
    ...
    tryThis: ['gh auth refresh -s project', 'gh project list --owner @me', 'github_ops(operation="project_list", owner="sandraschi")'],
    watchOut: ['Project numbers are scoped per owner — always pass the right org or user.'],
  },
  {
    key: 'github-packages',
    category: 'github',
    ...
    watchOut: ['Org package deletes may need admin — test on a sandbox package first.'],
  },
  {
    key: 'code-search',
    category: 'github',
    ...
    tryThis: ['github_ops(operation="search_repos", query="topic:mcp user:sandraschi")', 'github_ops(operation="code_find_repos", owner="sandraschi", extension="bak")'],
  },
  {
    key: 'gitingest',
    category: 'github',
    ...
    watchOut: ['Huge monorepos can exceed context — narrow with `subpath` when the tool supports it.'],
  },
  {
    key: 'fork-and-upstream',
    category: 'github',
    title: 'Forks and upstream sync',
    subtitle: 'Contributing to someone else\'s repo without losing your remotes.',
    body: [
    ...
    tryThis: ['git remote add upstream https://github.com/original/repo.git', 'git fetch upstream', 'git merge upstream/main'],
  },
  {
    key: 'breakfast',
    category: 'fleet',
  ...
  },
  {
    ...
    tryThis: ['fleet_morning_digest(fleet_repos="sandraschi/foo\\nsandraschi/bar")'],
  },
  {
    key: 'council-payload',
    category: 'fleet',
  ...
  },
  {
    ...
    tryThis: ['git_commit_message(diff="…", context="fix login redirect")', 'github_debug_workflow(workflow_name="ci", error_output="…", repo="o/r")'],
  },
  {
  ...
  },
  {
    ...
    tryThis: ['feat(web): add lecture tabs to help', 'fix(digest): handle gh pr comments as list'],
  },
  {
    key: 'force-push',
    category: 'git',
  ...
  },
  {
    ...
    tryThis: ['github_ops(operation="release_list", owner="…", repo="…")', 'git_branch(operation="tag_create", tag_name="v1.0.0")'],
  },
  {
    key: 'ack-stale-prs',
    category: 'fleet',
    ...
    <article className="rounded-xl border border-border bg-card/50 p-5 md:p-6 space-y-4">
      <header className="space-y-1">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-foreground">{lecture.title}</h2>
      ...
      {lecture.tryThis && lecture.tryThis.length > 0 && (
        <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-emerald-400 mb-2">Try this</p>
      ...
      {lecture.watchOut && lecture.watchOut.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-amber-400/90">Watch out</p>
...
export function Lectures() {
  ...
  const filtered = useMemo(() => {
    return LECTURES.filter((l) => {
      ...
      if (!q) return true;
      const hay = `${l.key} ${l.title} ${l.subtitle} ${l.body.join(' ')} ${(l.tryThis ?? []).join(' ')} ${(l.watchOut ?? []).join(' ')}`.toLowerCase();
    ...
    <div className="max-w-3xl space-y-5">
      <div>
        ...
        <p className="text-sm mt-1 text-muted-foreground leading-relaxed">
          Bite-sized explanations for git, GitHub, fleet maintenance, and agent patterns — readable first, commands second.
          More in <Link to="/help" className="text-sky-400 hover:underline">Help</Link> for server architecture.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg border border-border bg-card/40">
        ...
        />
        <span className="text-xs text-muted-foreground shrink-0">{filtered.length} lectures</span>
      </div>
      ...
      <div className="space-y-5">
        ...
        {filtered.length === 0 && (
          <p className="text-sm text-muted-foreground py-8 text-center">
            No match — try another category or a broader term like <span className="font-mono">merge</span> or{' '}
