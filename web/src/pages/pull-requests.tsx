...
import { GitPullRequest, Plus, X, Loader2, RefreshCw, Circle } from 'lucide-react';
...
export function PullRequests() {
  ...
  const createPR = async () => {
    ...
    setCreating(false);
    await githubOps('pr_create', { owner, repo, title: form.title, body: form.body, head_branch: form.head, base_branch: form.base, draft: form.draft });
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setCreating(c => !c)}
            ...
            style={{ background: creating ? 'var(--bg-3)' : 'var(--blue)', color: creating ? 'var(--text-muted)' : '#fff', border: `1px solid ${creating ? 'var(--border)' : 'var(--blue-dim)'}` }}>
            {creating ? <X size={12} /> : <Plus size={12} />}{creating ? 'Cancel' : 'New PR'}
          </button>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <input className="mono text-xs px-3 py-1.5 rounded outline-none w-36"
        ...
        <input className="mono text-xs px-3 py-1.5 rounded outline-none flex-1"
          style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', color: 'var(--text)' }}
      ...
      {creating && (
        <div className="rounded p-4 space-y-3" style={{ background: 'var(--bg-2)', border: '1px solid var(--border-2)' }}>
          <input className="w-full bg-transparent outline-none text-sm"
            style={{ borderBottom: '1px solid var(--border)', paddingBottom: 6, color: 'var(--text)' }}
            value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="PR title..." />
          <div className="flex items-center gap-2">
            <input className="mono text-xs px-3 py-1.5 rounded outline-none flex-1"
              style={{ background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--text)' }}
              value={form.head} onChange={e => setForm(f => ({ ...f, head: e.target.value }))} placeholder="head branch" />
            <span style={{ color: 'var(--text-dim)' }}>→</span>
            <input className="mono text-xs px-3 py-1.5 rounded outline-none w-28"
              style={{ background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--text)' }}
              value={form.base} onChange={e => setForm(f => ({ ...f, base: e.target.value }))} placeholder="base" />
          ...
          <textarea className="w-full bg-transparent outline-none text-xs mono resize-none" rows={3}
            ...
            value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))} placeholder="Description (optional)..." />
          <button onClick={createPR} className="px-4 py-1.5 rounded text-xs font-bold"
      ...
      {/* PR list */}
      {loading ? (
        <div className="flex justify-center py-12"><Loader2 className="animate-spin" size={20} style={{ color: 'var(--blue)' }} /></div>
      ) : error ? (
        ...
        <div className="rounded overflow-hidden" style={{ border: '1px solid var(--border)' }}>
          {prs.length === 0 ? (
            <div className="p-8 text-center text-sm" style={{ color: 'var(--text-dim)' }}>No {state} pull requests</div>
          ) : prs.map((pr, i) => (
            <div key={pr.number}
              className="flex items-start gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors"
              style={{ borderBottom: i < prs.length - 1 ? '1px solid var(--border)' : 'none' }}>
              <GitPullRequest size={14} className="mt-0.5 shrink-0"
                style={{ color: pr.isDraft ? 'var(--text-dim)' : pr.state === 'open' ? 'var(--green)' : 'var(--purple)' }} />
              <div className="flex-1 min-w-0">
                <a href={pr.url} target="_blank" rel="noreferrer"
                  className="text-sm font-medium hover:underline truncate block" style={{ color: 'var(--text)' }}>
                  {pr.title}
                  {pr.isDraft && (
                    <span className="ml-2 mono text-xs px-1.5 py-0.5 rounded"
                      style={{ background: 'var(--bg-3)', color: 'var(--text-dim)', border: '1px solid var(--border)' }}>Draft</span>
                  )}
                </a>
