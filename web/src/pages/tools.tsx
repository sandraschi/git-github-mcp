  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">MCP tool inspector — runtime from /api/capabilities</p>
        <button
          type="button"
          onClick={() => reload()}
      ...
      {caps && (
        <div className="grid sm:grid-cols-3 gap-3">
          {[
            ...
            <div key={String(k)} className="rounded-lg border border-border bg-card/40 px-4 py-3 text-center">
              <div className="text-xl font-semibold text-gh-green">{String(v)}</div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-border overflow-hidden">
          {tools.map((t) => (
            <div key={t.name} className="px-4 py-3 border-b border-border/80 last:border-0">
              ...
              {typeof t.operations === 'number' && (
                <div className="text-xs text-muted-foreground/80 mt-1">{t.operations} operations</div>
              )}
            </div>
          ))}
        </div>
