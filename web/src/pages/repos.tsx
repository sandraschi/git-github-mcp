...
export function Repositories() {
  ...
  const load = useCallback(() => {
    ...
    Promise.allSettled([
      (
        githubOps("repo_list", { limit: 50 }) as Promise<{
          result: { repos: GHRepo[] };
        }>
      )
        .then((d) => setGhRepos(d?.result?.repos ?? []))
        .catch(() => {}),
      (
    ...
    <div className="space-y-5 max-w-5xl">
        ...
        {localStatus?.success && localStatus.data && (
          <div className="flex items-center gap-4 mt-3 flex-wrap">
            ...
            <span className="mono text-xs flex items-center gap-1.5">
              ...
              <span
                style={{
                  color: localStatus.data.has_changes
                    ? "var(--amber)"
                    : "var(--green)",
                }}
              >
      ...
      <div className="space-y-3">
        ...
        {loading && ghRepos.length === 0 ? (
          ...
          >
            No repos — ensure{" "}
            <span className="mono text-white">gh auth login</span> is run
          </div>
        ) : (
          <div className="grid gap-2 md:grid-cols-2">
            {filtered.map((r) => (
              <div
                key={r.name}
                className="rounded p-4 group hover:border-slate-600 transition-colors"
                style={{
                  background: "var(--bg-2)",
                  border: "1px solid var(--border)",
                }}
