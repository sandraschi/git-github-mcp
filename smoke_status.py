...
try:
    r = subprocess.run(
        ["git", "--version"], capture_output=True, text=True, timeout=5, env=env, creationflags=NO_WINDOW
    )
    print(f"  -> rc={r.returncode} stdout={r.stdout.strip()!r}  ({time.perf_counter() - t:.3f}s)", flush=True)
except subprocess.TimeoutExpired:
    print("  -> TIMEOUT after 5s", flush=True)
...
t = time.perf_counter()
try:
    r = subprocess.run(["gh", "--version"], capture_output=True, text=True, timeout=5, env=env, creationflags=NO_WINDOW)
...
try:
    r = subprocess.run(
        ["gh", "auth", "token"], capture_output=True, text=True, timeout=5, env=env, creationflags=NO_WINDOW
    )
    tok = r.stdout.strip()
    print(
        f"  -> rc={r.returncode} token={'<present>' if tok else '<empty>'}  stderr={r.stderr.strip()[:80]!r}  ({time.perf_counter() - t:.3f}s)",
        flush=True,
    )
except subprocess.TimeoutExpired:
    print("  -> TIMEOUT after 5s", flush=True)
